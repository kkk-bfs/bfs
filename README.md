...
## 1.26.0903.204318（2026-09-03）

### 贴标停机与下一站瞄准

- **要点：** 近距下一站不会被刹车冲过数毫米后抬走；弹窗期间只写 `PresetStop`、窗口内不 Start，减少瞄错站。
- **窗口判断：** 与 `TryLabelling` 对齐，只有越过贴标窗口上沿才丢弃队首；接近段交给 PLC 自停，仅 `cy>ty` 时强制 Stop。
- **弹窗：** 下一站有效时写入下站停机距离；已在窗口内只瞄准不启动，确认关闭时同理。

### 多语言

- **要点：** 主界面检验信息区布种名称随当前语言显示（与疵点设置下拉一致）。
- **说明：** `DefaultDefectLibrary` 仍用库内原名；自定义布种名无词条时继续显示原文。切换语言后须重启。

---

## 1.26.0903.152800（2026-09-03）

### 布边检测

- **要点：** 布边任务会同步更新布料颜色 `clothcolor`，黑色背景破洞过滤与实时疵点图底色不再一直保持黑色。
- **路径：** `task_edge_detect` 计算布边时调用 `updateClothColor`，与旧 `slots_checkedgewidth` 一致。

---

## 1.26.0903.113249（2026-09-03）

### 灯带2

- **要点：** STM32 灯带开机只发一帧正确协议，不再沿用基类二次 `setLight` 导致全灭帧与亮度日志对不上。
- **同步关：** `EnableLightBandSync=0` 时开串口后发全灭 `SS01R000G000B000D0SS`。
- **同步开：** `EnableLightBandSync=1` 时 138 灯按 `LastSaveBrightness` 全亮。

---

## 1.26.0903.094316（2026-09-03）

### 灯带

- **要点：** 贴标停机亮灯只按有效疵点点亮，已过滤的误检不再占灯位。
- **筛选：** `filtertype=0` 才点亮；类型/精度/大小/布外等过滤疵点不参与灯带。

---

## 1.26.0903.083603（2026-09-03）

### 葡萄牙语与 Web 多语言

- **要点：** 补齐葡语检验字段与报警文案；Web 首页及子页切换语言会写回系统语言，检验单内嵌页跟随。
- **检验单字段：** 新增 `inspection_keys_pt_PT.csv`，葡语界面按葡语显示缸号/卷号等列名；缺文件回退英文。
- **报警：** `plc_alarm.json` 全量 `msg_pt`，葡语界面状态栏显示葡语报警。
- **Web 首页/子页：** 导航栏点语言立刻换词并写入 `CurrentLanguage`（`zh-CN`/`en`/`vi-VN`/`pt-PT`）；缸报告、疵点统计 iframe 跟 `?lang=` 与 `langchange`。设置/疵点/性能页动态文案随语言刷新。
- **切换后：** 桌面 Qt 界面、报表样式须重启软件后全部生效。

---

## 1.26.0902.215952（2026-09-02）

### 葡萄牙语界面

- **要点：** 设置页语言增加 Português（`pt-PT`），主界面、报表与 Web 调试页可切到葡萄牙语；切换后须重启软件。
- **疵点名称：** 与越南语相同，显示为「葡语 (英文)」，例如 `Mancha de óleo (Oil Dirty)`。
- **资源：** 安装包含 `Language_pt.xml`、报表词条 `reportlang-pt.dat`；缺词时回退英文。
- **检测设置 / 缸报告：** 网页补齐葡语文案，不再回退成中文或只显示英文克隆。
- **字号：** 葡语词较长，桌面复用越南语缩小字号样式；检测设置、缸报告、新增疵点页允许换行，避免按钮和标题溢出。
- **Web 首页及子页：** 导航栏切换语言立刻换词，不写回 `CurrentLanguage`；页面加载读软件当前语言；检验单内缸报告/疵点统计 iframe 跟随；设置/疵点/性能页动态文案随语言刷新。
- **报警：** `plc_alarm.json` 全量补 `msg_pt`，葡语界面状态栏显示葡语报警。

---

## 1.26.0902.173320（2026-09-02）

### 灯带（测试 Beta）

- **要点：** 本包用于 STM32 灯带2 台架/现场测试。V1 按 `LightBandDeviceType` 切换协议：1 创联容微二进制（默认），2 STM32 ASCII（UART 115200）。改类型后须重启软件。
- **协议 2：** 帧格式 `SS<count>R<r>G<g>B<b>D<mm>...SS\r\n`；灯间距 16.67mm、138 灯、总长 2300mm。贴标时按疵点中心距离(mm)点亮，不再用 X 步长做位图索引。
- **调试：** 灯光菜单「光带2」打开 `debuglightband2`，可按距离/灯号下发并看 STM32 回传。帮助页「贴标设置 → 灯带」按类型显示对应参数。
- **参数：** 共用串口、颜色、X 偏移、同步底光；类型 1 另需 `LightBandXStep`。串口勿与触摸板共用。现场设 `LightBandDeviceType=2` 后重启再测。

### 触摸板（测试 Beta，上一包已含）

- **要点：** 两次按压经串口上报后，新增疵点页自动框选 ROI 并弹出标签选择。

---

## 1.26.0902.144221（2026-09-02）

### 触摸板（测试 Beta）

- **要点：** 本包用于触摸板对接「新增疵点」台架/现场测试。两次按压经串口上报后，新增疵点页自动框选 ROI 并弹出标签选择。
- **设备：** 新增 `dev_touchpad`（115200 8N1 只收），V1 在 `IsEnableTouchPad=1` 时注册并开串口；调试页 `debugtouchpad`，帮助页可配偏移与有效宽长。
- **坐标映射：** `U/V/A/B` → 布面坐标 → 拼图像素 ROI；支持 `TouchPadOffsetU/V`、`TouchPadActualOffsetX/Y`、`TouchPadWidthMm/LengthMm`。
- **新增疵点页：** 轮询触摸板帧；触发后放大居中显示框并画控制点；未选标签关弹窗时保留触摸板 ROI，避免“收得到数据却看不见框”。
- **台架串口：** 勿用本机 Vulcan 的 COM10 冒充 com0com 对端；推荐验布 `TouchPadPortName=COM12`、模拟器发 `COM14`（或真实触摸板口）。灯带勿与触摸板共用同一 COM。

### 用户端测试说明书

- **触摸板：** 安装包附带 `docs/测试说明书/触摸板测试操作说明.pdf`，按步骤验证「按压 → 新增疵点自动画框」。

### 报喜鸟疵点

- **名称与类型：** 同步报喜鸟 detect Name / CustomerName，新增抽丝等类型，并更新默认疵点配置与维护 SQL。

### 贴标（近期一并入包）

- **到位停机：** 统一贴标窗口与关窗 HOLD；`cy` 接近 `ty` 时不再 Start，避免冲过目标后一直跑。
- **贴标顺序：** 按 ty 分组后再按幅宽 X 贴标，减少贴标机来回。

---

## 1.26.0902.143002（2026-09-02）

### 触摸板（测试 Beta）

- **要点：** 本包用于触摸板对接新增疵点现场/台架测试。两次按压串口上报后，新增疵点页自动框选 ROI 并弹出标签选择。
- **设备：** 新增 `dev_touchpad`（115200 8N1 只收），V1 在 `IsEnableTouchPad=1` 时注册并开串口；调试页 `debugtouchpad`，帮助页可配偏移/有效宽长。
- **坐标映射：** `U/V/A/B` → 布面坐标 → 拼图像素 ROI；支持 `TouchPadOffsetU/V`、`TouchPadActualOffsetX/Y`、`TouchPadWidthMm/LengthMm`。
- **新增疵点页：** 轮询触摸板帧；触发后放大居中显示框；未选标签关弹窗时保留触摸板 ROI，避免“收得到数据却看不见框”。
- **台架串口：** 勿用本机 Vulcan 的 COM10 冒充 com0com 对端；推荐验布 `TouchPadPortName=COM12`、模拟器发 `COM14`（或真实触摸板口）。
...
## 1.26.0901.171824（2026-09-01）

### 贴标（测试 Beta）

- **要点：** 本包用于贴标走布现场测试。到位冲过约 40～50mm 时提前停机；`cy` 接近 `ty` 时不再 Start，避免冲过目标后一直跑。
- **统一窗口：** 弹窗与确认/跳过关窗 HOLD 使用同一区间 `[ty−刹车距离, ty+允许偏差)`；刹车距离为 `max(50, 允许贴标偏差/2)` mm（常见 50mm）。
- **运行中：** 剩余距离已不足刹车，或已经越过 ty，强制 Stop，不再要求 PresetStop 大于 ty。
- **启动保护：** 走布 Start/Continue 在剩余过近时拒绝启动；界面「继续验布」同样拦住。站内贴标头走 Y 不受影响。

---

## 1.26.0901.154720（2026-09-01）

### 贴标

- **按布长分组：** 先去掉忽略/无效疵点，再按贴标 Y 从近到远分组；同一组为最小 ty 到 ty+布边最小间距，组内按幅宽从左到右贴，减少贴标机来回。
- **布边：** 每组第一张贴布边；后面的组只有走过 Y 才再贴边。

---

## 1.26.0901.101219（2026-09-01）

### 高级设置

- **参数备份：** 管理员可将系统参数按时间命名备份到 `configs` 目录，两次备份至少间隔 10 秒。
- **参数还原：** 选择备份文件即可还原全部参数，当前设置会被覆盖；备份与还原均写入操作日志。

---

## 1.26.0831.173540（2026-08-31）

### 灯光

- **暂停延迟关灯：** 点「暂停验布」后 1.5 秒再关灯；若贴标或 PLC 已恢复运行则跳过，避免黑图。继续验布时取消关灯并重新开灯。
- **停机延迟关灯：** V1 / V1 Pro 先关相机，1.5 秒后再关光源；若期间再次开检则取消关灯，避免关灯命令被 open 盖掉。

### 贴标

- **过近不启动：** 确认/跳过关窗时，若下一站剩余距离小于一个贴标窗口（PLC 刹不住），保持停机，不再下发 Start。

### 检测机型 — V1

- **生命周期：** init/uninit 带已初始化标志，ESC 打印机、色差仪、云端 MySQL 作为机型成员统一释放，避免重复初始化或漏释放。
