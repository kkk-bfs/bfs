...
## 1.26.0926.181458（2026-09-26）

### 灯带贴标

- **调试走灯带：** 开启灯带 2 贴标后，调试页「通知贴标 / 移动」走灯带串口，不再走 PLC；需先点贴标机归零。
- **走位后再贴：** 异步预走位未完成时，下一次贴标先等上次 `GO` 结束再发贴标命令。
- **异步预走位：** 整批贴完后，下一帧最小贴标 X 与布边停靠只下发 `GO`，贴标线程不等待到位，可与走布并行。
- **固件中止重贴：** 固件自行 `RL ABORT`（如掉标）时，自动重新出标后再发贴标，人工停机中止不重试。

## 1.26.0926.180114（2026-09-26）

### 灯带贴标

- **异步预走位：** 灯带 2 整批贴完后，下一帧最小贴标 X 与布边停靠均只下发 `GO`，贴标线程不等待到位，可与走布并行；下一次贴标前再等上次走位结束。
- **固件中止重贴：** 固件自行 `RL ABORT`（如掉标）时，自动重新出标后再发贴标，人工停机中止不重试。

## 1.26.0925.230118（2026-09-25）

### 灯带贴标

- **预走位异步：** 灯带 2 贴完一组或整批后，只下发 `GO` 到下一组/下一帧最小贴标 X，贴标线程不等待到位。走布与 X 走位可并行；下一次贴标前再等上次走位结束。

## 1.26.0925.220944（2026-09-25）

### 灯带贴标

- **下一帧预走位：** 灯带 2 整批贴完后，若队列还有下一帧，只下发 X 轴走到该帧最小贴标位置，贴标线程不等待，可与走布同时进行。同一批内部不再提前走位。没有下一帧时仍停到布边。

## 1.26.0925.211947（2026-09-25）

### 灯带贴标

- **下一组预走位：** 灯带 2 贴完一组后，X 轴先走到下一组最小贴标位置，下一站少空走。没有下一组时仍停到布边。

## 1.26.0925.193608（2026-09-25）

### 贴标

- **走布超时：** 贴标走 Y 按剩余距离、以 5 米/分钟估算总时限（最短 8 秒、最长 30 秒，另加 3 秒余量）。短距离不再在电机尚未起步时被掐断；已运行但 Y 超过 3 秒不动，仍判超时并停机。

### 退出

- **双面退出：** 设备在所属线程、事件循环还在时释放，再停线程。关程序不再卡在已经退出的线程上。

## 1.26.0925.182605（2026-09-25）

### 灯带贴标

- **调试页参数铺开：** 贴标深度、吸标深度、走位速度、加速度各自一行，查询和设置分开。收发日志可一键清理。
- **帮助页可配：** 可开关贴标走灯带 2，并设置贴标深度和失败重贴次数。启用后启动时贴标机归零。

### 主界面

- **疵点图与列表：** 疵点图和疵点列表同一行靠上对齐。

### 贴标

- **布边：** 首组始终贴布边；后续组仅在本组走过 Y 时贴，且在本组疵点贴完之后。

### 退出与加载

- **双面退出：** 设备在所属线程释放，关程序不再卡在已退出的线程上。
- **加载和退出页：** 背景和文字跟随灰、白、黑主题。

## 1.26.0925.162100（2026-09-25）

### 浅灰界面

- **车间浅灰主题：** 默认浅灰工业风，标题栏可在灰、黑、白之间切换。网页检测设置和 PLC 状态对比度已修正。
- **标题栏提示：** 标题栏按钮悬停高亮，并显示提示文字。

### 主界面

- **双线状态栏：** 双线状态改为两列加一行按钮，标题与检验单齐平。
- **布局 2：** 标题栏顶边对齐，状态按钮按固定比例缩放。
- **速度区：** 隐藏布边参数里的检测间隔，速度张力区增加左边距。
- **疵点设置表：** 表格字体与高级设置表一致。

### 运行统计

- **近七日统计：** 侧栏增加运行统计页，显示卷数、米数、检出、漏检、误检，并统计平均每卷用时。

### 灯带贴标

- **灯带走贴标：** 开启 `TicketControlByLightband2` 后，贴标 X 和贴标动作走灯带串口，启动时灯带回零。调试页可下发灯带贴标命令。

## 1.26.0924.113200（2026-09-24）

### 主界面布局

- **内容靠上对齐：** 检验单、疵点图、疵点列表和实时状态按内容高度靠上排列，不再被网格拉高。
- **疵点列表：** 标题栏高度固定，列表区域占满剩余高度。

## 1.26.0924.103831（2026-09-24）

### 双面两路相机

- **新机型：** 机型 26（双面）两路相机各自独立检测，结果分别进入第一路和第二路。
- **退出更稳：** 退出时设备在所属线程释放，避免关程序卡住。

### 实时状态栏

- **紧凑布局：** 码长、方向、张力、幅宽、速度、摆臂改为两列，按钮高度随状态区自适应，窄屏上不再被挤扁。

### 调试页

- **艾科相机配置：** 调试导航「相机与色差」增加艾科相机配置页，可现场查看和调整相机参数。

## 1.26.0924.084948（2026-09-24）

### 四分制实时显示

- **主界面扣分与评级：** 检验过程按米/码实时计算四分制扣分和Ⅰ–Ⅳ评级，并判定 OK/NG。
- **代替疵点类型：** 检验单信息右侧改为显示本段扣分、总扣分、百米/百平米扣分和合格线，现场可即时看结果。
...
- **分辨率一键同步：** X 分辨率可同步到 Y，保存前可核对。
- **算法参数：** 帮助页可改模型路径、置信度、YoloApi 版本、布边裁剪、最大输出框和多模型 NMS。
- **色差调试入口：** 与触摸板一样，标题栏按钮打开色差读取器调试页。
- **加载更快：** 钉钉截图改为懒加载；单行参数统一卡片边框，启用/禁用有绿/灰背景。
...
- **有效疵点数量：** 检验记录「结果」列后显示该卷有效疵点数量（检出且未忽略）；执行 `erpinterface/maolu/init_db.py` 或 `database_maintain.sql` 后生效。

### 误检图

- **按检验单归档：** 误检图保存到检验单号子目录，现场按卷查找更方便。

### 色差

- **CMC 与灰卡等级：** 色差支持 CMC 公式和灰卡等级，调试页与帮助可切换查看。

### 贴标与码长

- **不停机贴标：** 增加不停机贴标模块，码长 CurrentY 用快照估计，贴标对准更稳。

## 1.26.0918.220835（2026-09-18）

### 触摸板疵点框

- **中心用 X/Y：** 按压帧以 X/Y 为疵点中心（不再用 U/V 画框），有偏移时框仍对准实际位置。
- **宽高用 A/B：** A 为 X 向宽度、B 为 Y 向宽度；新增疵点 ROI、调试画布与标定同一口径。

### 毛路 ERP

- **新对接地址：** 衣智接口改为 `www.qingdaomorrow.com`。
- **扫码失败可见原因：** Token 或查询失败时把说明写到布料名称，界面能直接看到。
- **Token 失效自动重登：** 401 / token 过期时清缓存并重新登录后再查一次。
...
- **可选左下/左上原点：** 新增参数 `TouchPadOriginMode`：0 左下角（Y 向上）、1 左上角（Y 向下）；X 均从左向右。默认左下原点，与常见触摸板一致。
- **映射按原点换算：** 触摸板 U/V 到布面/拼图坐标按所选原点计算，避免 Y 方向上下颠倒。
- **帮助与调试可配置：** 帮助页与触摸板调试页均可切换并保存原点模式，调试画布随模式刷新。

## 1.26.0918.113334（2026-09-18）

### 触摸板标定

- **拼图点选实际坐标：** 调试页加载与新增疵点相同的看板拼图，点击疵点即可写入布面毫米坐标。
- **按压计算偏移：** 再点触摸板得到 U/V，自动计算并保存 `TouchPadOffsetU/V`，无需手工对表。
- **帮助页直达调试：** 界面设置可打开触摸板调试页。

## 1.26.0918.110035（2026-09-18）

### 触摸板新增疵点

- **坐标落到当前看板图：** 触摸板 U/V/A/B 先换成布面毫米，再映射到当前人工看板拼图像素后画框，与鼠标框选同一坐标系。
- **偏移可现场校准：** `TouchPadOffsetU/V` 校正安装偏差，`TouchPadActualOffsetX/Y` 校正拼图固定偏差；调试页可试算 ROI。
- **状态栏显示物理坐标：** 新增疵点页光标同时显示拼图像素和布面毫米。

### 关于与版本号

- **关于窗口跟安装包版本：** 启动时写入 `applicationVersion`，关于页打开即刷新，不再停留在编译 CustomUI 时的旧号。

### KnitAlgo v2609

- **检出框不丢：** 算法把 `isOutput` 清 0 后仍拷贝有效宽高的框，现场/testdetect 不再空检。
...
- **报告对齐现场表：** FGL FABI 报告（QA-WI-01-F01）按纸表列输出：破洞/撕裂只记 4 分、斜纹四列、灯箱 U35/U30，每卷头/中/尾三行可裁门幅。
- **导出同口径：** 检测记录导出 Excel 与报告同一套字段和计分（含页眉、表体、页脚），不再用通用导出列。

## 1.26.0914.092651（2026-09-14）

### 全图保存路径

- **换单后目录跟随检验单：** 新建检验单、ERP 建单、换卷或驳口自动建单后，全图（allimg）按当前 `CheckID` 刷新保存目录，不再继续写入上一张单的文件夹。
- **开机与存图时同步：** 检测线程打开时同步一次路径；存全图前若目录与当前检验单不一致会自动重刷。

## 1.26.0911.150132（2026-09-11）

### 标签打印机

- **重连后清报警：** 打印机未连接报警后，USB 重连或打印重试成功会同步 `isopen` 并立即清除 `ESC_PRINTER_ERROR`，状态栏不再一直显示「打印机未连接」。

## 1.26.0911.142408（2026-09-11）

### 布边检测

- **单轮廓外接矩形：** 只有一个轮廓时按该轮廓做 `boundingRect`，不再把整个轮廓列表传入，避免 OpenCV 异常导致布边失败。
- **算法异常不崩线程：** 图像布边抛异常时记录日志并回退空边，布边任务继续跑。

### 贴标 X 移动

- **ACK 优先：** `LabelState==0` 先于超时判断，避免同一拍把已成功当成失败。
- **等待加长：** `Labelling(2)` 等 ACK 500ms，其它命令 300ms。
- **连续贴标启动：** `Labelling(2)` 前先把 VW70 清 0，避免上一贴残留 1 导致 PLC 看不到上升沿。

## 1.26.0911.091504（2026-09-11）

### 布边检测

- **算法异常不崩线程：** 图像布边 `run_image_edge_detect` 抛异常时记录日志并回退空边，布边任务继续跑，避免整条检测流水线停住。
...
- **仅 EXE 更新包：** 只覆盖现场 `BullmerFabricScannerV1.exe`。

## 1.26.0910.235526（2026-09-10）

### 贴标 X 移动

- **连续贴标启动：** `Labelling(2)` 前先把 VW70 清 0，避免上一贴残留 1 导致 PLC 看不到上升沿、X 不移动。
- **贴完后移到布边外：** 开启后贴标头停在左/右布边外侧（`TicketFinishMoveDistance`，默认 200mm）。

### 安装包

- **仅 EXE 更新包：** 新增现场补丁，只覆盖 `BullmerFabricScannerV1.exe`，不替换界面、ERP、配置、模型或 DLL。

## 1.26.0910.225920（2026-09-10）

### 贴标完成回布边

- **贴完后移到布边外：** 开启 `TicketFinishMoveToEdgeEnable` 后，本批贴标成功结束会把贴标头移到左/右布边外侧（只走 X、不贴标），避免停在布面中间。
- **外侧距离可配：** `TicketFinishMoveDistance`（mm，取绝对值，默认 200）；左 `left − 距离`，右 `right + 距离`。模式 1/11 停左、2/12 停右；0/3/4 及其它选距当前 X 更近的一侧。入口：newhelp「功能-贴标/布边偏移设置」。
- **测试页可试走 X：** PLC 测试增加移动 X 按钮，目标位置会限制在行程范围内。

### 开始验布

- **靠近停机点仍可启动：** 开始验布不再因 CurrentY 接近 PresetStop 而拒绝启动；继续验布仍拦截过冲。

## 1.26.0910.223152（2026-09-10）
...
## 1.26.0910.214733（2026-09-10）

### 开始验布

- **靠近停机点仍可启动：** 开始验布不再因 CurrentY 接近 PresetStop 而拒绝启动；继续验布仍拦截过冲，避免 PLC 冲过目标。

### 实时界面

- **按钮样式：** 暂停/继续/选择按钮不再被控件级 QSS 清掉 `StyleSheet.css` 的 border-image，使能状态变化后仍保持原配色。
- **测试模式继续：** `IsTestMode` 开启时跳过红灯拦截，允许继续验布。

## 1.26.0910.154259（2026-09-10）

### 布头布尾不检布边

- **跳过半幅画面：** 开启布头布尾不检测时，布边任务与检测线程在布头/布尾区间不再计算布边，复用上一帧有效边或整幅宽，避免布未铺满污染 `DetectEdgeLeft/Right`。
- **规则统一：** 区间判定与检测跳过一致（开卷布头、无布停机后的布尾与下一卷布头、空单导布）。
- **读边加锁：** 检测裁剪/过滤读取布边改为 `edgeDataLock` 快照，避免与布边线程交叉写。
...
## 1.26.0910.141443（2026-09-10）

### 人工加疵点

- **贴标按钮：** 疵点列表可一键 X 向贴标（需启用 `IsEnableTicket`）；`tx` 限制在 `TicketXMinLimit`～`TicketXMaxLimit`。
- **确认弹窗：** 仅贴标使用页面风格确认框（多语言）；打码(打印标签)直接执行、不弹确认。
- **按钮样式：** 打码/贴标按钮加大配色，便于识别和点击。
...
- **贴标按钮：** 疵点列表可一键 X 向贴标（需启用 `IsEnableTicket`）；`tx` 限制在 `TicketXMinLimit`～`TicketXMaxLimit`。
- **确认弹窗：** 贴标使用页面风格确认框，支持多语言；打码按钮样式加大便于点按。
...
## 1.26.0909.233905（2026-09-09）

### 暂停关灯

- **减速不冲掉暂停态：** 点暂停后减速期间 PLC 仍报运行时，不再把 `machine_state` 误恢复为验布中，避免「延迟关灯跳过、当前已不是暂停」导致漏关灯。
- **策略统一：** 暂停态恢复规则抽到 `PauseMachineStatePolicy`，PLC 读状态与延迟关灯共用，避免两处逻辑漂移。
- **已停机也关灯：** 物理停或到位停后点暂停，仍进入软件暂停并延迟关灯，避免灯一直亮着。
- **停稳再关：** 点暂停后若布还在减速，会继续等待 PLC 报停再关光源；继续/开始/结束仍会取消关灯。

### 每日运行统计

- **一天一行：** Machine Performance 按自然日汇总，不再按布种拆行；筛选只保留起止日期。
- **合计列：** 总匹数、检验长度、检出疵点、检测准确率、停机时间、操作员、备注；导出 CSV 同样不含布种。

### 贴标停机

- **不再按速度改 VD240：** 开始/继续验布前不再按设定速度改写 PLC 停机过冲提前量（VD240），现场以配置页「停机偏移」为准。

### ERP 报表

- **疵点尺寸取整：** report3 疵点列表的位置 Y/X、长度、宽度改为整数显示，不再带一位小数。

## 1.26.0909.221043（2026-09-09）
...
## 1.26.0909.220330（2026-09-09）

### 暂停关灯

- **已停机也关灯：** 物理停或到位停后点暂停，不再因「已暂停」直接返回；仍进入软件暂停并延迟关灯，避免灯一直亮着。
- **停稳再关：** 点暂停后若布还在减速，会继续等待 PLC 报停再关光源；继续/开始/结束仍会取消关灯。
...
- **停稳再关：** 点暂停后若 1.5 秒内布还在减速，会继续等待 PLC 报停再关光源，避免灯一直亮着；继续/开始/结束仍会取消关灯。

### ERP 报表

- **疵点尺寸取整：** report3 疵点列表的位置 Y/X、长度、宽度改为整数显示，不再带一位小数。

## 1.26.0908.150329（2026-09-08）

### 每日运行统计

- **新页面：** 数据页导航增加「运行统计」，打开 Machine Performance 表，按日期和布种汇总当日验布结果。
- **统计列：** 总匹数、检验长度、检出疵点、检测准确率、停机时间、操作员、备注。
- **停机时间：** 按当天最早一卷到最晚一卷，把相邻两卷之间的间隔（本卷结束到下一卷开始）加总，单位分钟；同一天多个布种显示同一天停机，底部合计按日期去重。

### 新增疵点本月统计

- **本月卡片：** `debugadddefect` 增加本月份数/标注，并按自然月列出最近收集数量，点击某月即可筛选。
- **钉钉摘要：** 漏检目录扫描增加本月与按月汇总，和今日/近一周/近三个星期一起写入日报。
- **堆损坏修复：** 打开新增疵点统计页不再误删栈上参数，避免程序报堆损坏后退出。

### 实时状态按钮

- **按钮皮肤：** 实时状态四个操作按钮缩放时不再冲掉应用级皮肤，避免白底浅灰字几乎看不见。

### ERP 检验单

- **主题对齐：** 新建检验单与完成输入页跟随软件深色/浅色皮肤，控件样式与桌面一致。
- **五语界面：** 支持中文、英语、越南语、葡语、德语；打开对话框时带入当前系统语言。

### 布尾到位停机

- **多语言提示：** 布尾到位停机 MessageBox 补齐英/越/葡/德语文案，不再显示键名。

## 1.26.0908.121728（2026-09-08）
...
## 1.26.0908.084708（2026-09-08）

### 新增疵点目录统计

- **目录扫描：** 统计 `MissedImages` 今日、近一周、近三个星期的新增疵点份数与标注数，写入全局变量并纳入日报钉钉摘要。
- **调试页：** `debugadddefect` 按日期筛选列表，列出各类型添加数量；列表不再预加载原图，打开后再查看。
- **快照入库：** 扫描结果写入 Common.db，便于对照历史。

### 误检图归档

- **必须落 jpg：** 人工判定误检时，将原图拷贝到 `IgnoreImage`，不再只写 ALLImg 路径。
- **来源优先级：** 优先使用标注旁 `-origin` 副本，其次 `originImageRef`/ALLImg，再回退标注图。
- **任务覆盖：** V1 / V3 / 分时频闪保存任务共用同一归档逻辑。

### 报表语言

- **启动保护：** 启动时不再删除 `reportlang` 数据文件，避免语言模板被清掉。
...
- **机型覆盖：** `bfsmV1`、`bfsmV1Pro`、`bfsmV1Sp`、`bfsmV1SpPro` 通过 `LabellingFlowVersion=2` 选择 V2；默认值继续使用旧贴标任务。
- **统一任务链路：** V2 持续读取 `DetectionResultQueue` 队首批次，依据 `CurrentY`、目标 `ty` 和 `AllowableTicketOffset` 判断停机、到位、越过及后续动作。
- **统一 Y 容差：** V2 只使用 `AllowableTicketOffset`，不读取旧流程的 `EdgeTicketYMinRange`，不使用旧的 `/10`、`/2` 容差换算。
- **PLC 独立接口：** 新增 `PLCCommunication::BeginLabelingV2()` 及独立执行结果；旧 `BeginLabeling()` 继续作为 V0 入口，避免两套流程互相改变状态。
- **正常停机：** Y 未到位时设置 PLC 预设停机位并等待；进入允许窗口后由 PLC 自动停机，确认停机后才执行 X/ESC，不重复发送软件 `Stop()`。
- **严重越过：** `CurrentY - targetY > AllowableTicketOffset` 时，设备仍运行则只发送一次保护 `Stop()`；停机确认后仍执行 X/ESC，并要求人工调整标签位置，不丢弃有效疵点。
- **物理贴标：** 普通疵点按 ESC 打印（启用时）再执行 X 轴贴标；ESC 失败记录但继续 X，X 失败保留批次并进入重试。边缘贴标和大疵点多点贴标仍由 V2 PLC 接口处理。
- **批次保护：** 每批生成 `batchId`，消费前再次校验队首疵点身份；贴标失败、暂停、停止或人工调整未完成时不消费批次。

### 人工确认与数据库

- **UI 线程边界：** V2 任务通过 `Qt::QueuedConnection` 请求 UI，UI 线程创建并使用 `DefectAidedSelection::open()`；任务线程仅通过 `QWaitCondition` 等待结果，不直接操作 QWidget。
- **请求防串线：** 疵点确认和贴标控制均携带 `requestId` / `batchId`，重复或过期结果会被拒绝。
- **异步落库：** 人工结果创建 `RealDefect` 值副本后发送到常驻数据库保存任务，再唤醒贴标任务；数据库实际写入不阻塞 UI、X 轴或 ESC。
- **记录完整：** 选中疵点保存为正常记录，跳过疵点保存为忽略记录；直接关闭窗口按跳过并保持停机处理，人工确认数据不会因贴标失败而丢失。
- **失败重试：** 仅对未成功的 X 疵点重试，已完成疵点不重复贴标；取消重试时保留当前批次并保持停机。
- **严重越过调整：** X/ESC 完成后显示人工调整提示，只有点击“调整完成并继续”才消费当前批次并进入下一批次。

### 暂停、继续与物理启动

- **人工暂停：** 设置 `global.labelling.manualPause` 和 V2 内部暂停状态，暂停检测并按原逻辑延迟关灯；暂停期间不发送 `Start()`。
- **停机位附近暂停：** 设备已停机且队首疵点进入允许范围时，即使暂停已生效仍发起确认请求，避免弹窗丢失。
- **继续竞态修复：** 软件继续前后重新检查待确认请求；请求仍等待或尚未被 UI 接管时，不发送 `Continue`，避免弹窗刚显示就被 30 ms 定时器误判为启动跳过。
- **软件恢复标记：** 暂停与继续和任务请求创建同时发生时，未被 UI 接管的旧请求安全撤销，但队首批次保留，恢复后重新确认。
- **物理启动：** 通过 `physicalResumeSeq` 恢复 V2 暂停状态；已有弹窗仍沿用 `DefectAidedSelection` 的 30 ms 定时器自动跳过逻辑，物理启动与软件继续不混淆。
- **灯光安全：** 延迟关灯回调执行前再次检查设备状态；继续时先取消延迟关灯、恢复光源并等待稳定，降低黑图风险。
- **停止验布：** 先禁止新贴标、取消等待请求、关闭弹窗，再等待正在执行的 V2 PLC 贴标完成，最后停止 PLC 和机器。

### 兼容性与验证

- V0 继续使用旧任务、旧 `sig_show` 窗口链路和旧 `BeginLabeling()` 入口；V2 信号不会接入旧阻塞弹窗流程。
- V2 任务、UI 请求、PLC 执行、批次消费和数据库保存职责分离，四种机型共用同一套状态机。

### 贴标近距刹停

- **窗口内拒启动：** 下一站已在贴标窗口、或剩余距离小于刹车距离时保持停机并拒绝 `Start`，避免冲过允许贴标位置。
- **SetTy 回写：** 贴标完成写入目标 Y 后，等待 PLC `PresetStop` 回写再启动，避免仍读到上一站停机点而误拒走布。
- **弹窗时机：** 统一对话框关闭与停机偏移；开机尚未回零时不弹出疵点确认窗。
- **同 Y 批次：** 先按目标 Y 分组，再按 X 顺序贴标，保证走布方向正确。

### 灯带

- **STM32 ASCII：** 新增 `LightBandDeviceType=2`，按疵点横向位置点亮对应灯珠，可与原有灯带协议并存切换。

### 触摸板

- **新增疵点：** 斜面触摸板两次按压即可自动框选 ROI 并提交标签。

### 自动调光

- **只看最新带：** 亮度统计只用图像最新时间带；去掉新旧带 L 差超限跳过（`RoiBandSpreadMax`），避免整幅明暗差把调光卡住。
- **更快回馈：** 默认最新段比例 0.2、最小间隔 0.3 秒、硬件回读 100 ms，换卷或深浅变化时跟上且少来回修正。
- **防振荡：** 接近目标时光源反向加滞回（`LightReverseBias`），避免离散档位在目标两侧来回跳。
- **真实响应：** 按升/降光和灯值分段学习每档亮度变化，只用调光之后的新帧反馈，避免同一帧或旧帧误学习。
- **快速模式确认：** 大偏差需跨帧确认才放大步长，单帧噪声不再直接快调。
- **开关成对：** 检测设置页一个复选框同时写入 `IsAutoAdjustBrightnessMode` 与 `IsEnableLightSet`，两者共同决定自动调光是否生效。
- **物理恢复：** 暂停期间不消费物理启动序号，机器确认运行后再恢复光源，避免贴标暂停结束后灯不亮。

### 颜色采样

- **避开黑边：** RGB 均值固定取图像最前 200 行、横向中心左右各 200 像素，不依赖布边结果，避免两侧黑边把布料颜色拉低。

### 国际化

- **葡萄牙语：** 界面、报警、PLC 样式与报告语言支持 `pt-PT`；帮助页数字参数按英文小数点保存，避免逗号地区设置写坏数值。

### 安装与参数

- **管理员备份：** 高级设置可将系统参数导出为 JSON 并还原。

### ERP

- **报喜鸟：** 拆标准名与客户名；`K1-4` 显示污渍/油污，`K2-2` 显示粗纱，经纬疵/抽丝对齐客户名，新增横档 `K8-8`；已改过的客户名重跑脚本不再无条件覆盖。现场需重跑 `database_maintain.sql`。
- **合泰：** 导出增加计数字段，并提供 overlay 更新包。
